...
def gravity_for_k(k, L):
    epsilon = 1e-6
    # Przyjmujemy, że M i N są globalnymi stałymi, tak jak w oryginalnym kodzie
    j = np.arange(1, M)
    i = np.arange(1, N)
    I, J = np.meshgrid(i, j, indexing='ij')

    d = np.abs(k - I) + epsilon
    r = np.sqrt(d**2 + J**2)
...
def gravity_parallel(L):
    k_values = list(range(100, 400))

    num_workers = 4 #w moim przypadku kod dziala wtedy najszybciej jesli jest weicej to zazwyczaj bedzie dzialac wolniej niz bez multiprocessigniu
    with Pool(processes=num_workers) as pool:
        z_data = pool.starmap(gravity_for_k, [(k, L) for k in k_values])

    return z_data

if __name__ == '__main__':
    start = time.time()
    data_g = gravity_parallel(L)
    end = time.time()
    print(end - start)
